# 很棒的 Sitecore [![Awesome](https://awesome.re/badge.svg)](https://awesome.re)
...
- [内容中心](#content-hub)
- [内容中心一](#content-hub-one)
- [内容SDK](#content-sdk)
- [内容搜索](#content-search)
- [Data](#data)
- [数据交换框架](#data-exchange-framework)
- [Demo](#demo)
- [Deployment](#deployment)
- [Edge](#edge)
- [Editing](#editing)
- [扩展内容编辑器](#extending-content-editor)
- [Fields](#fields)
- [Forms](#forms)
...
- [项目资源文件](#item-resource-files)
- [Languages](#languages)
- [Logging](#logging)
- [Maintenance](#maintenance)
- [Marketplace](#marketplace)
- [Media](#media)
- [NET Core 无头](#net-core-headless)
- [ORM](#orm) 
- [Other](#other)
- [Packaging](#packaging)
- [Pipelines](#pipelines)
...
- [Sitecore 主机](#sitecore-host)
- [Sitecore 搜索](#sitecore-search)
- [Sitecore 发送](#sitecore-send)
- [SPE（Sitecore PowerShell 扩展）](#spe-sitecore-powershell-extension)
- [SPEAK](#speak)
- [SXA（Sitecore 体验加速器）](#sxa-sitecore-experience-accelerator)
- [Testing](#testing)
- [Templates](#templates)
- [Tooling](#tooling)
- [通用追踪器](#universal-tracker)
- [WebApi](#webapi) 
- [Workflows](#workflows)
- [xConnect 和 xDB](#xconnect-and-xdb)
- [星梦云](#xm-cloud)

> 请注意，**Awesome Sitecore** 列表仅对您可能受益的有用 GitHub 存储库进行分类和评论。
>
> 如果您正在寻找更全面的文章、博客文章、视频和问答列表，请参阅 [Sitecore 链接](https://Sitecore.Link) 知识库。

## 分析

- [Sitecore Goal Description](https://github.com/islaytitans/SitecoreGoalDescription) - 在体验配置文件中显示目标描述。
- [Sitecore Goal Conversion Demo](https://github.com/martinrayenglish/GoalConversions.Demo) - 演示如何获取访问者交互期间触发的目标，在会话结束后，可以将这些数据发送到外部系统，用于通过告知营销人员客户和潜在客户在其网站上执行的操作来帮助营销人员。
...
- [Sitecore.Boost](https://github.com/cardinal252/Sitecore.Boost) - 该存储库包含大量用于 Sitecore 平台性能改进的潜在补丁，其中大部分都在 Sitecore 的生产实例中运行。
- [Sitecore.Cleanup](https://github.com/martinrayenglish/Sitecore.Cleanup) - 该模块由 3 个代理组成，它们将监视每个事件队列、发布队列和历史记录表，以确保它们不超过设定的阈值。
- [SitecoreDXG: The Documentation Experience Generator](https://github.com/zkniebel/SitecoreDXG) - SitecoreUML 系列的 Sitecore 模板架构可视化和文档工具。利用与 StarUML（SitecoreUML 也基于此）相关的开源 API，SitecoreDXG 支持从具有 Sitecore 的 SitecoreUML 服务的任何现有 Sitecore 实例（均支持 PaaS 和非 PaaS）生成模板数据模型架构图和 HTML 文档。
...
- [ARM templates for Sitecore](https://github.com/Sitecore/Sitecore-Azure-Quickstart-Templates) - Sitecore 当前可用的所有 Azure 资源管理器模板。
- [Sitecore Azure Scripts](https://github.com/robhabraken/Sitecore-Azure-Scripts) - 与 Azure 相关的 PowerShell 脚本和 ARM 模板可用作满足您自己的脚本和需求的模板或样板。
...
- [CloudMediaLibrary](https://github.com/jammykam/Sitecore-CloudMediaLibrary) - 允许 Sitecore 媒体库通过云存储进行存储和提供服务。
...
- [ExperienceEditorCache](https://github.com/marek-musielak/Skillcore.ExperienceEditorCache) - 一个缓存模块，使 Sitecore 体验编辑器的使用更加愉快。
...
- [Dictionary Flush Partial Cache](https://github.com/jbluemink/DictionaryFlushPartialCache) - 当 Sitecore 词典项目发生更改时，会闪烁部分 HTML 缓存。

## CDP

- [Sitecore CDP Serializer](https://github.com/dylanyoung-dev/sitecore-cdp-serializer) - Node.js 工具旨在序列化硬盘驱动器上的 CDP 和个性化资产并将其恢复。
...
- [HabitatHome Commerce](https://github.com/Sitecore/Sitecore.HabitatHome.Commerce) - 在 XP 和 XC 上使用 SXA 遵循 Helix 架构原则构建的示例。
...
- [Short Confirmation Codes](https://github.com/dsolovay/ShortConfirmationCodes) - 提供随机生成的六字符确认代码（可配置），并在商业数据库中进行跟踪以避免冲突，因为较短的代码更容易通过支持电话进行通信，并且对于某些客户来说可能比开箱即用的 25 个字符确认更方便。
- [Sitecore Commerce Engine Fedex plugin](https://github.com/XCentium/SC-Plugin-FedEx) - 允许用户将 Fedex 集成为运输的履行方式。
- [Sitecore Commerce Engine USPS plugin](https://github.com/XCentium/SC-Plugin-USPS) - 允许用户将 USPS 集成为运输的履行方法。
...
- [XDT Transform playground)](https://akarzazi.github.io/xdt-playground) - 一个 Playground IDE，供您玩/测试 .NET 配置策略的 XDT 转换。
...
- [Sitecore MVP](https://github.com/Sitecore/MVP-Site) - 使用 Sitecore 10 和在容器中运行的 SXA 构建的真实 Sitecore MVP 网站的源代码。
- [Packer for Sitecore](https://github.com/asmagin/sitecore-packer) - 存储库包含用于本地 Sitecore 托管环境的 Packer 模板，其中包含 Windows 上的 IIS 和 SQL Server、SOLR 和 Sitecore 9.0 构建虚拟机映像以及 VirtualBox 的 Vagrant 框（由 Chef 提供）。
- [Docker SOLR with SSL](https://github.com/LaubPlusCo/docker-solr-ssl) - 一个简单的设置，使用生成的证书在运行带有 SSL 的 solr 的主机上设置 Linux 容器。
- [Sitecore Docker Tools](https://github.com/sitecore/docker-tools) - 一组实用程序，用于改善开发人员在 Docker 环境中运行 Sitecore 时的体验。这是一个包含可在 Sitecore 容器构建期间使用的开发脚本和入口点的映像，以及一个 PowerShell 模块，其中包含在 Sitecore 容器主机上用于初始化 Sitecore Docker 环境的功能。
...
- [Test Sitecore Packages](https://github.com/michaellwest/test-sitecore-packages) - 提供了一种快速方便地安装软件包作为 docker 映像构建的一部分或在容器启动后安装的方法。对于需要验证构建工件在 XM 的普通实例中工作的模块开发人员或团队很有用。支持 XM、SPE、SXA 和任何自定义模块 zip/scwdp 或通用 zip 文件。

## 内容中心

- [Content Hub CLI](https://github.com/Sitecore/content-hub-cli) - 包含 Sitecore Content Hub CLI 源代码的存储库。
...
- [Sitecore.ContentHub.Twitter](https://github.com/josedbaez/Sitecore.ContentHub.Twitter) - 演示如何从 Sitecore 内容中心 CMP 发送推文。
...
- [Starter Kit for Content Hub with Next.js and GraphQL](https://github.com/konabos/Next.js-Starter-kit-using-GraphQL-and-Sitecore-Content-Hub-Content-as-a-Service) - Content Hub 无头入门套件。
...
- [Content Hub ONE examples](https://github.com/Sitecore/contenthubone-examples) - 一组使用 Content Hub One 的各种示例。
- [Sitecore.Demo.CHONE](https://github.com/Sitecore/Sitecore.Demo.CHONE) - 由 Sitecore 演示解决方案团队构建的所有 Content Hub ONE 演示的存储库，包括 PLAY Media Next.js 网站和 PLAY Media 手机应用程序。
- [Content Hub ONE Next.JS Starter Kit](https://github.com/Sitecore/content-hub-one-nextjs-starterkit) - 入门套件包含一些帮助功能，例如处理 GraphQL JSON 输出转换为 HTML 的富文本、媒体字段和引用。它还包含一个小示例实现，开发人员可以使用它来启动他们的客户项目。

## 内容SDK

- [Sitecore Content SDK](https://github.com/Sitecore/content-sdk) - 所有 Sitecore Content SDK 包和模板的源代码可帮助您开始使用 Sitecore Content SDK。
- [XM Cloud Front End Application Starter Kits](https://github.com/Sitecore/xmcloud-starter-js) - 此内容 SDK 存储库包含多个 Next.js 入门套件以及用于 Sitecore XM 云开发的 SPA 入门 monorepo（其中包括节点代理应用程序和 SPA 入门应用）。

## 内容搜索

- [Sitecore spatial geojson polygons](https://github.com/josedbaez/sitecore-spatial-geojson-polygons) - 实现 Linq 扩展，以便能够查询在 Solr 索引中拦截 GeoJson 多边形的点。该演示提供了一个带有空间字段的模板，以及几个来自 OpenStreetMap 的包含多边形的示例项目，覆盖了一些城市；控制器渲染从查询字符串中读取纬度和经度并返回找到的项目的多边形（例如大本钟的坐标返回伦敦）。
...
- [Sitecore SolrProxy](https://github.com/Antonytm/Sitecore.SolrProxy) - 如果您只能访问 Sitecore CM，则可以使用该工具轻松访问 Solr 管理控制台。
- [Common Sitecore ContentSearch extensions](https://github.com/LaubPlusCo/LaubPlusCo.Common.ContentSearch) - Sitecore contentSearch 的常见扩展；下面的 PopulateSolrSchema 管道处理器可确保 Sitecore 中所有安装的语言都添加到 solr 托管架构中。
- [Sitecore Computed Search](https://github.com/martinrayenglish/Sitecore-Computed-Search) - 索引搜索开发加速器，允许开发人员在索引上声明计算搜索字段，该索引可以捕获和存储某个范围内项目的目标字段值以及基于作为其表示的一部分的特定模板的项目的字段值。此外，可以使用搜索提升将特定权重应用于这些计算字段，以便可以在搜索结果中提升内容匹配的搜索项。
...
- [Gutters for Sitecore Data Exchange Framework](https://github.com/KayeeNL/Sitecore.DataExchange.Gutters) - DEF 的 3 个槽的集合，为树中项目的上下文项提供快速解决方案：“ItemDisabled”、“PipelineBatch”和“PipelineStep”。
- [Data Exchange Framework Docs](https://github.com/Sitecore/Data-Exchange-Framework-Docs) - 使用 Sphinx 生成的 Sitecore 数据交换框架文档。

## 演示
- [Sitecore Habitat](https://github.com/Sitecore/Habitat) - Habitat 是一个基于 Helix 架构原则构建的 Sitecore 解决方案示例。不再更新或维护，并被 Sitecore Helix Examples 取代。
...
- [Sitecore User Group UK](https://github.com/steviemcg/scuguk) - JAMstack 架构的演示，使用 Git 作为单一事实来源，使用 Netlify 进行持续部署，并使用 Gatsby v2 构建 CDN 分发。
...
- [Sitecore Developer Portal](https://github.com/Sitecore/developer-portal) - Sitecore 开发人员门户是使用 Next.js、Typescript 和 Tailwind CSS 构建的，并托管在 Vercel 上。它使用静态站点生成（SSG）在构建时创建所有页面。它还利用增量静态再生 (ISR) 在页面内容发生更改时自动更新应用程序。许多页面都是用 Markdown 编写的，并在构建时转换为 HTML。使用的图像在 Sitecore DAM 中进行管理并发布到 CDN。
- [Play Summit](https://github.com/Sitecore/Sitecore.Demo.XmCloud.PlaySummit) - 播放 Summit 演示，其中包括 XM Cloud、Content Hub DAm、CMP、带有 Versel 托管的 Next.js 等。
- [Verticals](https://github.com/Sitecore/Sitecore.Demo.XMCloud.Verticals) - 专注于 XM Cloud 内容和站点管理功能的无头多站点解决方案。包括针对特定行业的可轻松定制的示例站点。
- [Developer Portal](https://github.com/Sitecore/developer-portal) - Sitecore 开发人员门户使用 Next.js、Typescript、Tailwind CSS 构建并托管在 Vercel 上，使用静态站点生成在构建时创建所有页面。它还利用增量静态再生 (ISR) 在页面内容发生更改时自动更新应用程序。由于大部分页面内容都是用 Markdown 编写并在构建时转换为 HTML，因此图像在 Sitecore DAM 中使用和管理，
发布到 CDN。

## 部署

- [PostDeploySteps](https://github.com/jst-cyr/NonlinearPostDeploySteps) - 与 TDS 一起使用的部署后步骤。 1) 触发部署后步骤的TDS示例项目； 2) 示例 Web 项目以显示部署步骤中的拉动； 3) 可以添加到解决方案中的 DeployStep 类库。
- [Sitecore Devops with AppVeyor](https://github.com/steviemcg/Sitecore.Devops.AppVeyor) - 示例解决方案展示了如何开发开源 Sitecore 模块并将其与 AppVeyor（托管在云中的持续交付解决方案）连接。
...
- [Browse Command](https://github.com/ParTech/Browse-Command) - 添加一个命令，允许用户在新的浏览器窗口中打开 Sitecore 内容树中的项目。 Sitecore 不提供这种开箱即用的功能，除了具有进入预览模式效果的预览命令之外，因此给定的模块将执行相同的操作但不进入预览模式。
...
- [Environment Styler for Sitecore](https://github.com/jammykam/Environment-Styler-for-Sitecore) - Sitecore 登录屏幕和标题功能区上特定于环境的样式（和文本）。
- [InsertOptionsLoophole](https://github.com/TwentyGotoTen/InsertOptionsLoophole) - 防止 Sitecore 用户绕过插入选项。
- [ScopeToThis](https://github.com/ianjohngraham/Coreblimey.ScopeToThis) - 允许在 Sitecore 内容编辑器树上使用 Visual Studio 的“范围到此”功能。
- [DeviceEditorShortcuts](https://github.com/MartinMiles/DeviceEditorShortcuts) - 设备编辑器的工作效率得到了提高，如果设置为该特定组件，则显示数据源，并通过弹出式单击直接预览该数据源。
...
- [CrossDatabaseTreeListField](https://github.com/ivansharamok/CrossDatabaseTreeListField) - 扩展的 Treelist 字段，支持跨数据库引用。
- [LimitedText Field](https://github.com/ParTech/LimitedText-Field-Controls) - 向 Sitecore 内容编辑器添加了两种新字段类型：单行文本限制和多行文本限制。两者都继承了现有文本字段控件的所有功能，并添加了配置字段允许的最大值的可能性，还通知用户在编辑时剩余的字符数。
- [CustomFields](https://github.com/AlexanderDavyduk/CustomFields) - 精选的 NameValueDropLists、NameValueDropListsField、NameValueDroplist、NameValueDroplistField、SortableMultilist、SortableMultilistField、TimeZonesDropList 字段。
...
- [Sitecore-Forms-Extensions](https://github.com/bartverdonck/Sitecore-Forms-Extensions) - 向表单创建器添加一些功能，例如发送电子邮件、时间跨度验证、订阅列表、条件、Azure Blob 存储提供程序等等。
- [WFFM Conversion Tool](https://github.com/afaniuolo/WFFM-Conversion-Tool) - 一个控制台应用程序，提供自动化解决方案，用于将营销人员 Web 表单 (WFFM) 项目及其数据转换并迁移到 Sitecore 表单。
...
- [Sitecore GraphQL Import](https://github.com/jbluemink/Sitecore-GraphQL-Import) - 控制台应用程序 C# 演示，展示了 Sitecore GraphQL API 使用的各种功能，例如检索 Sitecore 项目、获取网站列表、插入示例项目和上传媒体文件。

## 果酱栈

- [Next.js Starter kit using GraphQL and Sitecore Experience Edge for Content Hub](https://github.com/konabos/Next.js-Starter-kit-using-GraphQL-and-Sitecore-Content-Hub-Content-as-a-Service) - 这是在 React 上使用 NextJS 的 Sitecore 体验 Edge 内容即服务方法的一个很好的示例，并使用了最新内容中心的演示实例。
- [Uniform, JSS and Next.js starter kit](https://github.com/uniformdev/sitecore-jss-nextjs-starterkit) - Uniform、JSS 和 Next.JS 的入门套件，包含内容项和所需的配置文件，非常适合用来启动普通项目。

## JSS

- [sugcon-2019-jss-examples](https://github.com/chaturangar/sugcon-2019-jss-examples) - SugCon 2019 jss 示例。
- [SitecoreQL](https://github.com/kmazzoni/SitecoreQL) - 例如，Sitecore 的 GraphQL 实现，您可以查询 Sitecore 的内容搜索 API。
...
- [How to GraphQL](https://github.com/kamsar/howtographql) - 一个完整的堆栈教程网站，用于了解有关 GraphQL 的所有知识。
...
- [jss21.4-nextjs-storybook7.4](https://github.com/jflheureux/jss21.4-nextjs-storybook7.4) - Sitecore JSS 21.4 Next.js 示例应用程序，其中逐步添加 Storybook 7.4，并通过提交历史记录公开。

## 无头

- [Sample](https://github.com/uniformdev/sitecore-jss-nextjs-starterkit) - 无头示例 TODO
- [Angular JSS starter kit for XM Cloud](https://github.com/Sitecore/jss/tree/release/22.0.0/packages/create-sitecore-jss/src/templates/angular-xmcloud) - 适用于 XM Cloud 的 Sitecore JSS Angular 入门套件应用程序。

## 螺旋

- [Sitecore Helix Documentation](https://github.com/Sitecore/Helix.Docs) - 在 Sitecore 中针对 Sitecore Helix 进行开发的官方指南和推荐实践。
- [Sitecore Helix Examples](https://github.com/Sitecore/Helix.Examples) - 跨各种工具和业务场景的 Sitecore Helix 实践演示；旨在展示比现有示例更广泛的实现类型/要求。
- [Helixbase](https://github.com/muso31/Helixbase) - 适用于新建项目的基于 Sitecore Helix 的解决方案。
- [Sitecore Foundation](https://github.com/Avanade/SitecoreFoundation) - Sitecore框架遵循Helix模块化架构设计原则。包括大量的Feature和Foundation层模块，以及Project层可重用的Common。
...
- [Helix Publishing Pipeline](https://github.com/richardszalay/helix-publishing-pipeline) - 允许 Helix 解决方案作为单个单元发布，并自动包含模块的内容（如视图和配置补丁）。它还包含有关本地开发部署的优化和指导。由于它扩展了标准的 Web 发布管道，因此它应该通过 Visual Studio 或命令行与任何支持的目标（包、文件系统、Azure、Docker）配合使用。
...
- [sitecore-icon-build](https://github.com/richardszalay/sitecore-icon-build) - MSBuild 扩展，用于构建 Sitecore 图标 zip 存档并将其包含在已发布的网站中。
- [Sitecore Icons](https://github.com/Antonytm/sitecore-icons) - 可安装的 1800 多个 FA 和 2500 多个 MUI 图标 X 4 种颜色（黑色、蓝色、绿色、红色）的集合。
...
- [Sitecore Item Translator](https://github.com/adoprog/Sitecore-Item-Translator) - 项目翻译器模块与谷歌翻译无缝集成，因此只需单击按钮，您就可以将文本翻译成谷歌翻译支持的任何语言。
...
- [RabbitMQ.GEFL.Appender for Sitecore](https://github.com/asmagin/Sitecore.Logger.RabbitMQ.GelfAppender) - 用于 Sitecore.Logger 的 RabbitMQ 记录器适配器的实现。
- [Logging To Logentries](https://github.com/jammykam/Sitecore.Logentries) - 配置您的应用程序以登录 Logentries（支持 NLog、Log4net、Serilog 等）。
- [Namics.Foundation.Logger](https://github.com/merkle-open/Namics.Foundation.Logger) - 提供一组静态方法用于通过灵活的选项进行日志记录
...
- [Sitecore Instance Manager](https://github.com/Sitecore/Sitecore-Instance-Manager) - 传奇的 Sitecore Instant Manager，现在支持 9.x 版本。
- [Sifon](https://github.com/MartinMiles/Sifon) - 备份恢复工具支持 XC 和远程机器操作，具有出色的可插拔接口。 Sifon 还有一个非常强大的扩展系统，其插件涵盖了日常活动的许多方面。

## 市场
- [Sitecore Marketplace Starter](https://github.com/Sitecore/marketplace-starter) - 用于构建 Marketplace 扩展的入门模板。它演示了五个扩展点：自定义字段、仪表板小部件、全屏、页面上下文面板和独立。每个扩展点都有自己的 UI 并与 Sitecore Marketplace SDK 集成。
- [Google Analytics](https://github.com/Sitecore/marketplace-google-analytics) - XM Cloud 环境中的无缝 Google Analytics 集成。该模块直接在您的 Sitecore 体验中提供实时分析数据可视化，包括页面浏览量和活跃用户指标。
- [Icon Picker](https://github.com/Sitecore/marketplace-icon-picker) - 市场图标选择器 - 一个示例应用程序，展示如何创建自定义字段扩展。
...
- [Shrink](https://github.com/robhabraken/shrink) - 类似于硬盘驱动器的磁盘使用统计查看器，深入了解媒体库的使用情况，还显示哪些项目正在使用和发布，以便您可以轻松找到不必要占用数据库空间的媒体项目，并为您提供多种清理媒体库的方法！
- [Dianoga](https://github.com/kamsar/Dianoga) - Sitecore 媒体库的自动图像优化器可以完全自动地将 Sitecore 提供的图像大小减少 8-70%。当请求媒体图像时，Dianoga 在将图像数据放入 Sitecore 媒体缓存后立即自动对图像数据运行 mozjpeg、PNGOptimizer、SVGO 或 WebP。
...
- [TemplateModelHelper](https://github.com/lowedown/TemplateModelHelper) - 用于处理生成的 Sitecore 模板模型（通过 Glass.Mapper 或其他框架映射）的辅助方法，其主要目标是使开发人员能够轻松地使用生成的模板模型查询 Sitecore 数据库。
- [TDS-T4-Model-Generation](https://github.com/Sitecore/TDS-T4-Model-Generation) - TDS 的 T4 模型生成。
- [Sitecore.CodeGenerator](https://github.com/ParTech/sitecore.codegenerator) - 使用 T4 模板生成 Glass Mapper 接口，无需 TDS。
- [Synthesis](https://github.com/blipson89/Synthesis) - Sitecore 的对象映射框架，与传统 Sitecore 开发相比，可以在更短的时间内开发更可靠、更可维护的网站。它是一个强类型模板对象生成器，对于具有 Sitecore 或传统 .NET 背景的开发人员来说很容易理解。它与 Sitecore MVC（通过 Synthesis.Mvc 包）巧妙地集成，作为视图渲染模型提供程序和控制器渲染的 IoC 依赖项。
...
- [BLAZOR + SITECORE](https://github.com/GoranHalvarsson/SitecoreBlazor) - 允许您运行 Sitecore 应用程序的客户端，同样遵循 HELIX 概念（干净的方式）。
...
- [Sitecore Processor Dependency Injection](https://github.com/coreyasmith/Sitecore.ProcessorDi) - 用于演示 Sitecore 管道处理器中的依赖项注入的简单项目。
...
- [Publish Viewer](https://github.com/mikeedwards83/Glass.PublishViewer) - 允许您监控 Sitecore Publishing CM 服务器上的发布队列，查看其当前状态，并在需要时取消发布作业（即排队时间、发布作业启动时间、队列中的持续时间、发布的项目数、发布项目的平均时间、来自已发布作业的所有消息）。
- [Sitecore Power Publish](https://github.com/robhabraken/sitecore-power-publish) - 发布按钮强制发布项目，无论发布限制中项目选项卡上的可发布选项的状态如何。 1) 此功能不会发布您网站中未发布的链接页面，而只会发布项目字段中使用的媒体库项目和数据源等资源，因为需要这些资源才能正确显示您要发布的项目。此方法甚至会发布当前项目使用的模板和布局（如果尚未完成）。 2) 取消发布按钮使您只需单击一下即可取消发布项目。它将更改发布限制，因为它取消选中“项目”选项卡上的“可发布”选项，然后使用不带子项目的完整重新发布来发布该项目。 3) 发布状态按钮显示所有发布目标的发布状态。如果发布目标是最新的，则会显示绿点。如果该项目已发布到发布目标，但该项目随后发生了更改，则会显示一个橙色点（因此它已发布，但不是最新的）。如果当前项目根本不存在于发布目标中，则会显示红点。此功能使内容编辑者能够针对每个发布目标详细检查内容是否已发布。

## 规则

- [Page Rules](https://github.com/marek-musielak/Marek.Musielak.PageRules) - 为访问者浏览的每个页面创建任何自定义 Sitecore 规则。用户尚未阅读条款和条件页面并想查看竞赛页面？该页面应该只能在特定日期之后访问？您想使用 GEO IP 数据自动更改主页的语言吗？您还没有为您的页面准备好翻译吗？有了 Sitecore 规则引擎，天空才是极限！
...
- [Sitecore Adaptive Rules](https://github.com/adamconn/sitecore-adaptive-rules) - 添加对自适应规则的规则引擎支持（当条件和操作的属性相互依赖时，这些规则很有用）。

## 软件开发工具包
- [JSS](https://github.com/Sitecore/jss) - Sitecore JavaScript Services SDK 的官方存储库。
- [Content SDK](https://github.com/Sitecore/content-sdk) - 所有 Sitecore Content SDK 包和模板的源代码可帮助您开始使用适用于 XM Cloud 的 Sitecore Content SDK。
- [ASP.NET Core SDK](https://github.com/Sitecore/ASP.NET-Core-SDK) - 适用于 Sitecore DXP 和 XM Cloud 的官方开源 ASP.NET Core SDK。
- [Marketplace SDK](https://github.com/Sitecore/sitecore-marketplace-sdk) - Sitecore Marketplace SDK 托管三个主要包：客户端应用程序（在 iframe 内运行）、核心 SDK 和 XMC 模块，它们扩展了系统的功能。
...
- [ASP.NET 2.0 Membership Database as Identity Server User Store](https://github.com/Sitecore/sitecore-identityserver-contrib-membership) - 根据包含现有系统用户数据的现有 ASP.NET 2.0 成员资格数据库验证用户登录名和密码。
- [Certz](https://github.com/michaellwest/certz) - 基于 .net 7 构建的实用程序并编译为独立的 exe，旨在简​​化证书管理，作为 mkcert 的更好替代品。
...
- [Unicorn](https://github.com/SitecoreUnicorn/Unicorn) - Sitecore 的实用程序，通过将 Sitecore 项目的序列化副本与代码一起写入磁盘，解决在 Sitecore 实例之间移动模板、渲染和其他数据库项目的问题 - 这样，给定代码库的必要数据库项目的副本会在源代码管理中伴随它。
...
- [Sitecore Search Starter Kit](https://github.com/Sitecore/Sitecore-Search-TS-SDK-Starter-Kit) - 网站示例 此存储库包含使用 Sitecore Search JS SDK 与 Sitecore 搜索服务集成并支持事件跟踪的内容网站的示例实现。
  
## Sitecore 发送

- [Sitecore Send Postman Collection](https://github.com/neilkillen/SitecoreSendPostmanCollection) - 将 Sitecore Send API Blueprint 规范转换为 Open API 以在 Postman 中使用而派生的 postman 集合。

## SPE（Sitecore PowerShell 扩展）
...
- [Speak 3 starter template](https://github.com/Mitya88/SitecoreSpeak3StarterProject) - Speak 3 使用 Angular CLI 版本 1.2.7 生成的入门模板。
...
- [SXA Styleguide](https://github.com/markvanaalst/SXA.Styleguide) - 一个基于 SXA 的教育网站，有助于解释内部工作原理并说明最佳实践。
- [SXA.Styleguide.Frontend](https://github.com/markvanaalst/SXA.Styleguide.Frontend) - 存储库包含运行站点所需的所有前端代码。它由两部分组成，SXA 主题和网站上使用的 Scriban 模板。此存储库中使用的文件夹结构模仿 SXA Creative Exchange 输出，因此您可以找到主题和单独的 Scriban 模板的源。
...
- [SXA.HealthCheck](https://github.com/alan-null/SXA.HealthCheck) - 用于确定 SXA 站点健康状态的 Power Shell 脚本由验证步骤组成，每个步骤检查 SXA 解决方案中的不同内容并提供可能的解决方案。
- [Global Field Validator](https://github.com/JeffDarchuk/SxaGlobalFieldValidator) - SXA 模块用于在站点级别而不是模板字段级别验证字段。
...
- [Terminal DevEx Improvements](https://github.com/Sitecore/Windows-Terminal-DevEx-improvements) - Sitecore 品牌的 Windows 终端和 VS Code 主题和配置文件可解锁用于使用 XM Cloud 的有用工具，例如自动完成、命令历史列表和目录改进。
...
- [Sitecore Endpoints](https://github.com/MartinMiles/Sitecore.Endpoints) - 准备好挑选和使用以 Helix 功能形式实现的 Sitecore 服务客户端和 Sitecore WebApi 示例。
...
- [Sitecore Services Client Publish](https://github.com/peplau/SscPublish) - 一种 Web API 方法，用于安全触发项目或树的 Sitecore 发布。
...
- [XConnectTutorial](https://github.com/jst-cyr/XConnectTutorial) - 基于 Sitecore 文档的教程存储库，根据“入门”教程和其他优秀文档中 Martina Welander 的代码，引导用户完成与 xConnect API 的常见交互。
...
- [Experience Generator](https://github.com/Sitecore/xGenerator) - 使用可配置模式为 Sitecore 网站生成“逼真”的流量。
- [xconnect-odata-proxy](https://github.com/ianjohngraham/xconnect-odata-proxy) - 一个简单的 Node.js 代理，用于访问 Sitecore 9 中 xConnect 的 oData API。
...
- [Next.js Styleguide for XM Cloud](https://github.com/sitecorelabs/XmCloudNextJsJssStyleguide) - 该解决方案旨在帮助开发人员快速学习并开始使用 Sitecore 容器、Sitecore Next.js SDK 和 Sitecore 内容序列化。
- [Play Summit](https://github.com/Sitecore/Sitecore.Demo.XmCloud.PlaySummit) - 播放 Summit 演示，其中包括 XM Cloud、Content Hub DAM、CMP、带有 Versel 托管的 Next.js 等。
- [XM Cloud Starter Kit](https://github.com/sitecorelabs/xmcloud-foundation-head) - 旨在帮助开发人员快速学习和入门 XMCLoud + SXA + Next.js 的解决方案。
- [XM Cloud Introduction](https://github.com/Sitecore/XM-Cloud-Introduction) - 由 Sitecore 的技术营销团队管理的一系列 XM Cloud 站点的代码库：新的 MVP 网站以及 3 个 SUGCON 活动网站。
- [FEaaS BYOC Example](https://github.com/Sitecore/feaas-nextjs-example) - 该存储库演示了使用最佳实践的 BYOC 组件示例。
- [Sitecore GraphQL Import](https://github.com/jbluemink/Sitecore-GraphQL-Import) - 控制台应用程序展示了 Sitecore GraphQL API 的各种功能，例如检索 Sitecore 项目、获取网站列表、插入示例项目和上传媒体文件。
- [Verticals](https://github.com/Sitecore/Sitecore.Demo.XMCloud.Verticals) - 专注于 XM Cloud 内容和站点管理功能的无头多站点解决方案。包括针对特定行业的可轻松定制的示例站点。
- [Node XM Cloud Proxy](https://github.com/Sitecore/jss/tree/release/22.0.0/packages/create-sitecore-jss/src/templates/node-xmcloud-proxy) - 节点代理应用程序可实现与 XM Cloud 的 Next.js 入门套件中的后端功能和特性相同的功能。引入是为了支持新的 Angular 入门套件。该代理旨在与所有 SPA 框架配合使用，也可用于运行 React 和 Vue 应用程序，为未来为其他前端 JavaScript 框架构建的 JSS 入门套件奠定基础。
