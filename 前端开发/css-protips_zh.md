...
# CSS 提示 [![Awesome](https://awesome.re/badge-flat.svg)](https://awesome.re)

一系列技巧可帮助您提高 CSS 技能。

> [!提示]
> 对于其他精彩列表，请查看 [@sindresorhus](https://github.com/sindresorhus/) 精选的[很棒列表](https://github.com/sindresorhus/awesome/) 列表。
...
1. [使用 CSS 重置](#use-a-css-reset)
1. [继承 `box-sizing`](#inherit-box-sizing)
1. [使用`unset`而不是重置所有属性](#use-unset-instead-of-resetting-all-properties)
1. [使用 `:not()` 在导航上应用/取消应用边框](#use-not-to-applyunapply-borders-on-navigation)
1. [检查字体是否本地安装](#check-if-font-is-installed-locally)
1. [将 `line-height` 添加到 `body`](#add-line-height-to-body)
1. [为表单元素设置 `:focus`](#set-focus-for-form-elements)
1. [垂直居中任何东西](#vertically-center-anything)
1. [使用`aspect-ratio`代替高度/宽度](#use-aspect-ratio-instead-of-heightwidth)
1. [逗号分隔列表](#comma-separated-lists)
1. [使用负`nth-child`选择项目](#select-items-using-male-nth-child)
1. [使用 SVG 作为图标](#use-svg-for-icons)
1. [使用“Lobotomized Owl”选择器](#use-the-lobotomized-owl-selector)
1. [对纯 CSS 滑块使用 `max-height`](#use-max-height-for-pure-css-sliders)
1. [等宽表格单元格](#equal-width-table-cells)
1. [使用 Flexbox 摆脱 Margin Hacks](#get-rid-of-margin-hacks-with-flexbox)
1. [使用带有空链接的属性选择器](#use-attribute-selectors-with-empty-links)
1. [使用 `:is()` 更好地控制特异性](#control-specificity-better-with-is)
1. [样式“默认”链接](#style-default-links)
1. [内在比率框](#intrinsic-ratio-boxes)
1. [风格破碎图像](#style-broken-images)
1. [使用 `rem` 进行全局调整；使用 `em` 进行局部尺寸调整](#use-rem-for-global-sizing-use-em-for-local-sizing)
1. [隐藏未静音的自动播放视频](#hide-autoplay-videos-that-arent-muted)
1. [使用 `:root` 作为灵活类型](#use-root-for-flexible-type)
1. [在表单元素上设置“font-size”以获得更好的移动体验](#set-font-size-on-form-elements-for-a-better-mobile-experience)
1. [使用指针事件控制鼠标事件](#use-pointer-events-to-control-mouse-events)
1. [在用作间距的换行符上设置“display: none”](#set-display-none-on-line-breaks-used-as-spacing)
1. [使用 `:empty` 隐藏空 HTML 元素](#use-empty-to-hide-empty-html-elements)
1. [使用 `margin-inline` 代替 `margin`](#use-margin-inline-instead-of-margin)

### 使用 CSS 重置

CSS 重置有助于在不同浏览器之间强制执行样式一致性，并为样式元素提供干净的状态。有很多重置模式可供查找，或者您可以使用更简化的重置方法：
...
> [!提示]
> 如果您遵循下面的[继承 `box-sizing`](#inherit-box-sizing) 提示，您可能会选择不在 CSS 重置中包含 `box-sizing` 属性。
