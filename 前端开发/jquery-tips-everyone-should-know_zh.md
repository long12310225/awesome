# 每个人都应该知道的 jQuery 技巧 [![Awesome](https://awesome.re/badge-flat.svg)](https://awesome.re)

一系列帮助您提高 jQuery 游戏水平的简单技巧。
...
1. [检查 jQuery 是否已加载](#checking-if-jquery-loaded)
...
1. [检查图像是否加载](#checking-if-images-are-loaded)
1. [自动修复损坏的图像](#fix-broken-images-automatically)
1. [使用 AJAX 发布表单](#post-a-form-with-ajax)
1. [悬停时切换类](#toggle-classes-on-hover)
1. [禁用输入字段](#disabling-input-fields)
1. [停止加载链接](#stop-the-loading-of-links)
1. [缓存 jQuery 选择器](#cache-jquery-selectors)
1. [切换淡入淡出/幻灯片](#toggle-fadeslide)
1.[简单手风琴](#simple-accordion)
1. [使两个div高度相同](#make-two-divs-the-same-height)
1. [在新选项卡/窗口中打开外部链接](#open-external-links-in-new-tabwindow)
1. [通过文本查找元素](#find-element-by-text)
1. [可见性变化触发](#trigger-on-visibility-change)
1. [AJAX 调用错误处理](#ajax-call-error-handling)
1. [链插件调用](#chain-plugin-calls)
1. [按字母顺序对列表项目排序](#sort-list-items-alphabetically)
1. [禁用右键](#disable-right-click)

### 使用“noConflict()”

jQuery 使用的“$”别名也被其他 JavaScript 库使用。为了确保 jQuery 不会与不同库的“$”对象冲突，请在文档开头使用“noConflict()”方法：

```javascript
jQuery.noConflict();
```

现在，您将使用“jQuery”变量名而不是“$”来引用 jQuery 对象（例如，“jQuery('div p').hide()”）。如果同一页面上有多个版本的 jQuery（不推荐），则可以使用“noConflict()”为特定版本设置别名：
