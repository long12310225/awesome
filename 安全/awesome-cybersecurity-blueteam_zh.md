# 出色的网络安全蓝队 [![Awesome](https://awesome.re/badge-flat2.svg)](https://awesome.re)

> 为网络安全蓝队提供的一系列精彩资源、工具和其他闪亮的东西。

[网络安全蓝队](https://en.wikipedia.org/wiki/Blue_team_(computer_security)) 是由个人组成的小组，他们识别信息技术系统中的安全缺陷、验证安全措施的有效性并监控系统以确保所实施的防御措施在未来仍然有效。虽然不是排他性的，但该列表严重偏向于[自由软件](https://www.gnu.org/philosophy/free-sw.html)项目，而不是专有产品或企业服务。对于令人反感的 TTP，请参阅 [awesome-pentest](https://github.com/fabacab/awesome-pentest)。

♥ 欢迎您的贡献和建议。 (✿◕‿◕)。请查看[贡献指南](CONTRIBUTING.md) 了解更多详细信息。本作品根据 [知识共享署名 4.0 国际许可证](http://creativecommons.org/licenses/by/4.0/) 获得许可。
...
**[取消对警察的资助。](https://defundthepolice.org/)**

## 内容

- [自动化与会议](#automation-and-convention)
  - [代码库和绑定](#code-libraries-and-bindings)
  - [安全编排、自动化和响应 (SOAR)](#security-orchestration-automation-and-response-soar)
- [云平台安全](#cloud-platform-security)
  - [分布式监控](#distributed-monitoring)
  - [Kubernetes](#kubernetes)
  - [服务网格](#service-meshes)
- [通信安全（COMSEC）](#communications-security-comsec)
- [DevSecOps](#devsecops)
  - [应用程序或二进制强化](#application-or-binary-hardening)
  - [合规性测试和报告](#compliance-testing-and-reporting)
  - [依赖混乱](#dependency-confusion)
  - [Fuzzing](#fuzzing)
  - [政策执行](#policy-enforcement)
  - [供应链安全](#supply-chain-security)
- [Honeypots](#honeypots)
  - [Tarpits](#tarpits)
- [基于主机的工具](#host-based-tools)
  - [Sandboxes](#sandboxes)
- [身份和 AuthN/AuthZ](#identity-and-authnauthz)
- [事件响应工具](#incident-response-tools)
  - [红外管理控制台](#ir-management-consoles)
  - [证据收集](#evidence-collection)
- [网络周边防御](#network-perimeter-defenses)
  - [防火墙设备或发行版](#firewall-appliances-or-distributions)
- [操作系统发行版](#operating-system-distributions)
- [网络钓鱼意识和报告](#phishing-awareness-and-reporting)
- [准备训练和兵棋推演](#preparedness-training-and-wargaming)
  - [参与后分析和报告](#post-engagement-analysis-and-reporting)
- [安全配置](#security-configurations)
- [安防监控](#security-monitoring)
  - [端点检测和响应 (EDR)](#endpoint-detection-and-response-edr)
  - [网络安全监控（NSM）](#network-security-monitoring-nsm)
  - [安全信息和事件管理 (SIEM)](#security-information-and-event-management-siem)
  - [服务和性能监控](#service-and-performance-monitoring)
  - [威胁狩猎](#threat-hunting)
- [威胁情报](#threat-intelligence)
  - [Fingerprinting](#fingerprinting)
  - [威胁签名包和集合](#threat-signature-packages-and-collections)
- [Tor Onion 服务防御](#tor-onion-service-defenses)
- [传输层防御](#transport-layer-defenses)
  - [覆盖网络和虚拟专用网络 (VPN)](#overlay-and-virtual-private-networks-vpns)
- [基于 macOS 的防御](#macos-based-defenses)
- [基于Windows的防御](#windows-based-defenses)
  - [活动目录](#active-directory)

## 自动化与会议

- [Ansible Lockdown](https://ansiblelockdown.io/) - 精选的信息安全主题 Ansible 角色集合，这些角色均经过审查和积极维护。
- [Clevis](https://github.com/latchset/clevis) - 用于自动解密的可插入框架，通常用作 Tang 客户端。
- [DShell](https://github.com/USArmyResearchLab/Dshell) - 用 Python 编写的可扩展网络取证分析框架，可以快速开发插件来支持网络数据包捕获的解析。
- [Dev-Sec.io](https://dev-sec.io/) - 服务器强化框架提供各种基线安全配置的 Ansible、Chef 和 Puppet 实现。
- [Password Manager Resources](https://github.com/apple/password-manager-resources) - 协作、众包数据和代码，使密码管理变得更好。
- [peepdf](https://eternal-todo.com/tools/peepdf-pdf-analysis-tool) - 可编写脚本的 PDF 文件分析器。
- [PyREBox](https://talosintelligence.com/pyrebox) - Python 可编写脚本的逆向工程沙箱，基于 QEMU。
...
- [python-sandboxapi](https://github.com/InQuest/python-sandboxapi) - 用于构建与恶意软件沙箱的集成的最小、一致的 Python API。
- [python-stix2](https://github.com/oasis-open/cti-python-stix2) - 用于序列化和反序列化结构化威胁信息表达 (STIX) JSON 内容的 Python API，以及用于常见任务的更高级别 API。

### 安全编排、自动化和响应 (SOAR)

另请参阅[安全信息和事件管理 (SIEM)](#security-information-and-event-management-siem) 和 [IR 管理控制台](#ir-management-consoles)。
...
- [Kata Containers](https://katacontainers.io/) - 使用轻量级虚拟机保护容器运行时，这些虚拟机的感觉和性能类似于容器，但使用硬件虚拟化技术作为第二层防御提供更强大的工作负载隔离。
...
- [gVisor](https://github.com/google/gvisor) - 应用程序内核，用 Go 编写，实现了 Linux 系统表面的大部分，以在应用程序和主机内核之间提供隔离边界。

### 分布式监控

另请参阅[§服务和性能监控](#service-and-performance-monitoring)。

- [Cortex](https://cortexmetrics.io/) - 为 Prometheus 提供水平可扩展、高可用、多租户的长期存储。
...
另请参阅 [Kubernetes-Security.info](https://kubernetes-security.info/)。

- [KubeSec](https://kubesec.io/) - Kubernetes 清单的静态分析器可以在本地运行，作为 Kuberenetes 准入控制器，或作为自己的云服务。
- [Kyverno](https://kyverno.io/) - 专为 Kubernetes 设计的策略引擎。
- [Linkerd](https://linkerd.io/) - 超轻型 Kubernetes 特定服务网格，为 Kubernetes 应用程序增加了可观察性、可靠性和安全性，而无需对应用程序本身进行任何修改。
- [Managed Kubernetes Inspection Tool (MKIT)](https://github.com/darkbitio/mkit) - 查询和验证托管 Kubernetes 集群对象以及集群内运行的工作负载/资源的几个常见的与安全相关的配置设置。
...
- [kube-hunter](https://kube-hunter.aquasec.com/) - 开源工具，从集群外部（“攻击者的视角”）或集群内部运行一组针对 Kubernetes 集群中的安全问题的测试（“猎人”）。
...
另请参阅 [ServiceMesh.es](https://servicemesh.es/)。
...
另请参阅[传输层防御](#transport-layer-defenses)。

- [GPG Sync](https://github.com/firstlookmedia/gpgsync) - 在组织或团队的所有成员之间集中和自动化 OpenPGP 公钥分发、撤销和更新。
- [Geneva (Genetic Evasion)](https://censorship.ai/) - 新颖的实验性遗传算法，针对民族国家级审查员制定基于数据包操作的审查规避策略，以提高其他被阻止内容的可用性。
...
- [Checkov](https://www.checkov.io/) - Terraform（基础设施即代码）的静态分析有助于检测 CIS 策略违规并防止云安全配置错误。
- [Cilium](https://cilium.io/) - 开源软件，用于透明地保护使用 Docker 和 Kubernetes 等 Linux 容器管理平台部署的应用程序服务之间的网络连接。
- [Clair](https://github.com/coreos/clair) - 用于探测通过应用程序容器（例如 Docker）映像引入的漏洞的静态分析工具。
...
- [Gauntlt](http://gauntlt.org/) - 例行持续集成期间的渗透测试应用程序构建管道。
...
- [SonarQube](https://sonarqube.org) - 持续检查工具，可在自动化测试期间提供详细报告，并针对新引入的安全漏洞发出警报。
- [Trivy](https://github.com/aquasecurity/trivy) - 针对容器和其他工件的简单而全面的漏洞扫描器，适合在持续集成管道中使用。
- [Vault](https://www.vaultproject.io/) - 用于通过统一界面安全访问 API 密钥、密码或证书等机密的工具。
- [git-crypt](https://www.agwa.name/projects/git-crypt/) - git中的透明文件加密；您选择保护的文件在提交时会加密，在签出时会解密。
...
- [Egalito](https://egalito.org/) - 二进制重新编译器和检测框架，可以完全反汇编、转换和重新生成普通 Linux 二进制文件，专为二进制强化和安全研究而设计。
...
- [OpenSCAP Base](https://www.open-scap.org/tools/openscap-base/) - 库和命令行工具（“oscap”）用于根据 SCAP 基线配置文件评估系统，以报告扫描系统的安全状况。

### 依赖混乱

另请参阅[§供应链安全](#supply-chain-security)。
...
- [snync](https://github.com/snyk-labs/snync) - 预防并检测您是否容易受到依赖性混淆供应链安全攻击。
...
另请参见[§依赖混淆](#dependency-confusion)。

- [Grafeas](https://grafeas.io/) - 打开工件元数据 API 来审核和管理您的软件供应链。
...
- [CanaryTokens](https://github.com/thinkst/canarytokens) - 自托管蜜币生成器和报告仪表板；演示版可在 [CanaryTokens.org](https://canarytokens.org/) 获取。
- [Kushtaka](https://kushtaka.org) - 适用于资源不足的蓝队的可持续一体式蜜罐和蜜币协调器。
- [Manuka](https://github.com/spaceraccoon/manuka) - 开源情报 (OSINT) 蜜罐，用于监视威胁参与者的侦察尝试并为蓝队生成可操作的情报。
...
- [Artillery](https://github.com/BinaryDefense/artillery) - 蜜罐、文件系统监视器和警报系统的组合，旨在保护 Linux 和 Windows 操作系统。
- [Crowd Inspect](https://www.crowdstrike.com/resources/community-tools/crowdinspect-tool/) - 适用于 Windows 系统的免费工具旨在提醒您存在可能通过网络进行通信的恶意软件。
...
另请参阅[安全编排、自动化和响应 (SOAR)](#security-orchestration-automation-and-response-soar)。

- [CIRTKit](https://github.com/opensourcesec/CIRTKit) - 基于 Viper 构建的可编写脚本的数字取证和事件响应 (DFIR) 工具包。
- [Fast Incident Response (FIR)](https://github.com/certsocietegenerale/FIR) - 网络安全事件管理平台可以轻松创建、跟踪和报告网络安全事件。
- [Rekall](http://www.rekall-forensic.com/) - 先进的取证和事件响应框架。
- [TheHive](https://thehive-project.org/) - 可扩展的免费安全事件响应平台，旨在让 SOC、CSIRT 和 CERT 的工作变得更加轻松，并与 MISP 紧密集成。
- [threat_note](https://github.com/defpoint/threat_note) - Defense Point Security 构建的 Web 应用程序使安全研究人员能够添加和检索与其研究相关的指标。
...
另请参阅[维基百科：路由器和防火墙发行版列表](https://en.wikipedia.org/wiki/List_of_router_and_firewall_distributions)。

- [IPFire](https://www.ipfire.org/) - 基于 GNU/Linux 的强化路由器和防火墙发行版，源自 IPCop。
- [OPNsense](https://opnsense.org/) - 基于 FreeBSD 的强化防火墙和路由平台，源自 pfSense。
- [pfSense](https://www.pfsense.org/) - FreeBSD 防火墙和路由器发行版从 m0n0wall 分叉出来。

## 操作系统发行版

- [Computer Aided Investigative Environment (CAINE)](https://caine-live.net/) - 意大利 GNU/Linux 实时发行版，预打包了大量数字取证和证据收集工具。
- [Security Onion](https://securityonionsolutions.com/) - 用于入侵检测、企业安全监控和日志管理的免费开源 GNU/Linux 发行版。
- [Qubes OS](https://qubes-os.org/) - 在 Xen 虚拟机管理程序项目之上构建的桌面环境，该项目在自己的虚拟机中运行每个最终用户程序，旨在提供严格的安全控制，以限制任何成功的恶意软件利用的范围。

## 网络钓鱼意识和报告

另请参阅 [awesome-pentest § 社会工程工具](https://github.com/fabacab/awesome-pentest#social-engineering-tools)。
...
- [NotifySecurity](https://github.com/certsocietegenerale/NotifySecurity) - Outlook 加载项用于帮助您的用户向安全团队报告可疑电子邮件。
- [Phishing Intelligence Engine (PIE)](https://github.com/LogRhythm-Labs/PIE) - 有助于检测和响应网络钓鱼攻击的框架。
- [Swordphish](https://github.com/certsocietegenerale/swordphish-awareness) - 该平台允许创建和管理（虚假）网络钓鱼活动，旨在培训人们识别可疑邮件。
- [mailspoof](https://github.com/serain/mailspoof) - 扫描 SPF 和 DMARC 记录以查找可能允许电子邮件欺骗的问题。
- [phishing_catcher](https://github.com/x0rz/phishing_catcher) - 可配置脚本，用于使用 [CertStream](https://certstream.calidog.io/) 服务在证书透明度日志 (CTL) 中按域名监视可疑 TLS 证书的颁发。
...
- [Atomic Red Team](https://atomicredteam.io/) - 用于执行测试安全控制的简单、自动化测试库。
...
- [Drool](https://www.dns-oarc.net/tools/drool) - 重放抓包文件中的 DNS 流量并将其发送到指定服务器，例如用于模拟对 DNS 的 DDoS 攻击并测量正常的 DNS 查询。
- [DumpsterFire](https://github.com/TryCatchHCF/DumpsterFire) - 模块化、菜单驱动、跨平台工具，用于为蓝队演习和传感器/警报映射构建可重复、延时、分布式安全事件。
- [Infection Monkey](https://www.guardicore.com/infectionmonkey/) - 开源违规和攻击模拟 (BAS) 平台可帮助您验证现有控制并确定攻击者如何利用您当前的网络安全漏洞。
...
- [Stratus Red Team](https://stratus-red-team.cloud/) - 针对云环境以精细且独立的方式模拟进攻性攻击技术；想想“云的 Atomic Red Team™”。
...
- [Tsunami](https://github.com/google/tsunami-security-scanner) - 具有可扩展插件系统的通用网络安全扫描器，可高置信度地检测高严重性漏洞。
- [VAST](https://github.com/tenzir/vast) - 免费开源网络遥测引擎，用于数据驱动的安全调查。
...
- [Prelude SIEM OSS](https://www.prelude-siem.org/) - 开源、无代理 SIEM 具有悠久的历史和多个商业变体，具有安全事件收集、规范化以及来自任意日志输入和众多流行监控工具的警报。

### 服务和性能监控

另请参阅 [awesome-sysadmin#monitoring](https://github.com/n1trux/awesome-sysadmin#monitoring)。

- [Icinga](https://icinga.com/) - 对 Nagios 进行模块化重新设计，提供可插拔的用户界面以及一组扩展的数据连接器、收集器和报告工具。
...
- [DeepBlueCLI](https://github.com/sans-blue-team/DeepBlueCLI) - 通过 Windows 事件日志进行寻线的 PowerShell 模块。
- [GRR Rapid Response](https://github.com/google/grr) - 事件响应框架专注于远程实时取证，由安装在资产上的 Python 代理和基于 Python 的服务器基础设施组成，使分析师能够快速分类攻击并远程执行分析。
...
- [Logging Made Easy (LME)](https://www.cisa.gov/resources-tools/services/logging-made-easy) - 免费开放的日志记录和保护监控解决方案服务。
- [MozDef](https://github.com/mozilla/MozDef) - 自动化安全事件处理流程并促进事件处理程序的实时活动。
- [PSHunt](https://github.com/Infocyte/PSHunt) - PowerShell 模块旨在扫描远程端点以查找危害迹象或调查它们以获取与这些系统状态相关的更全面的信息。
- [PSRecon](https://github.com/gfoss/PSRecon) - 类似 PSHunt 的工具，用于分析远程 Windows 系统，还可以生成其结果的独立 HTML 报告。
- [PowerForensics](https://github.com/Invoke-IR/PowerForensics) - 基于 PowerShell 的多合一平台可执行实时硬盘取证分析。
- [Redline](https://www.fireeye.com/services/freeware/redline.html) - 由 FireEye, Inc. 提供的免费软件端点审核和分析工具，提供基于主机的调查功能。
...
- [AttackerKB](https://attackerkb.com/) - 免费的公共众包漏洞评估平台，可帮助确定高风险补丁应用的优先级并消除漏洞疲劳。
...
- [Sigma](https://github.com/Neo23x0/sigma) - SIEM 系统的通用签名格式，提供开放的签名格式，允许您以简单的方式描述相关日志事件。
...
- [YARA](https://github.com/VirusTotal/yara) - 该工具旨在（但不限于）帮助恶意软件研究人员识别和分类恶意软件样本，被描述为文件模式和签名的“模式匹配瑞士军刀”。

### 指纹识别

- [HASSH](https://github.com/salesforce/hassh) - 网络指纹识别标准，可用于识别特定的客户端和服务器 SSH 实现。
...
- [FireEye's Red Team Tool Countermeasures](https://github.com/fireeye/red_team_tool_countermeasures) - 用于检测使用 FireEye 自己的红队工具进行的攻击的 Snort 和 YARA 规则集合，该规则在 FireEye 于 2020 年 12 月披露漏洞后首次发布。
...
- [YARA Rules](https://github.com/Yara-Rules/rules) - 该项目涵盖了 IT 安全研究人员对拥有一个单一存储库的需求，其中不同的 Yara 签名被编译、分类并尽可能保持最新。
...
- [Certbot](https://certbot.eff.org/) - 免费工具，可通过配置各种 Web 和电子邮件服务器软件的插件自动从 [LetsEncrypt Root CA](https://letsencrypt.org/) 颁发和续订 TLS 证书。
...
另请参阅 [drduh/macOS-Security-and-Privacy-Guide](https://github.com/drduh/macOS-Security-and-Privacy-Guide)。
...
- [Santa](https://github.com/google/santa) - 在 macOS 的允许/拒绝列表系统中跟踪恶意或友好的二进制文件。
...
- [NotRuler](https://github.com/sensepost/notruler) - 在尝试破坏 Microsoft Exchange 服务器时，检测 [Ruler](https://github.com/sensepost/ruler) 攻击工具使用的客户端规则和启用 VBScript 的表单。
- [Sandboxie](https://www.sandboxie.com/) - 免费开源通用 Windows 应用程序沙箱实用程序。
- [Sigcheck](https://docs.microsoft.com/en-us/sysinternals/downloads/sigcheck) - 根据 Microsoft 的[证书信任列表 (CTL)](https://docs.microsoft.com/en-us/windows/desktop/SecCrypto/certificate-trust-list-overview) 审核 Windows 主机的根证书存储。
- [Sticky Keys Slayer](https://github.com/linuz/Sticky-Keys-Slayer) - 从主机名列表建立 Windows RDP 会话，并扫描辅助工具后门，如果发现则发出警报。
...
- [PlumHound](https://github.com/PlumHound/PlumHound) - 通过利用其寻路引擎来识别 Active Directory 安全漏洞，在持续的安全生命周期中更有效地使用 BloodHoundAD。
