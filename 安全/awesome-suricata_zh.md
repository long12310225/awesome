# 很棒的苏里卡塔 [![Awesome](https://awesome.re/badge-flat2.svg)](https://awesome.re)
...
- [输入工具](#input-tools)
- [输出工具](#output-tools)
- [操作、监控和故障排除](#operations-monitoring-and-troubleshooting)
- [编程库和工具包](#programming-libraries-and-toolkits)
- [仪表板和模板](#dashboards-and-templates)
- [开发工具](#development-tools)
- [文档和指南](#documentation-and-guides)
- [分析工具](#analysis-tools)
- [规则集](#rule-sets)
- [规则/安全内容管理和处理](#rulesecurity-content-management-and-handling)
- [插件和扩展](#plugins-and-extensions)
- [使用 Suricata 的系统](#systems-using-suricata)
- [Training](#training)
- [模拟与测试](#simulation-and-testing)
- [数据集](#data-sets)
- [Misc](#misc)
...
- [suricata-kafka-output](https://github.com/Center-Sun/suricata-kafka-output) - Suricata Eve Kafka Suricata 6 输出插件。
- [suricata-redis-output](https://github.com/jasonish/suricata-redis-output) - Suricata 7 的 Suricata Eve Redis 输出插件。
- [Meer](https://github.com/quadrantsec/meer) - Suricata / Sagan 的“假脱机程序”。
...
- [suricata_exporter](https://github.com/corelight/suricata_exporter) - 用 Go 编写的简单 Prometheus 导出器导出从 Suricata 套接字抓取的统计指标。
- [Triagewall](https://github.com/aaronphifer/triagewall) - Suricata 警报的本地 LLM 分类层，使用可调预过滤器和 Ollama 分类器来减少警报量。
...
- [gonids](https://github.com/google/gonids) - 用于解析 Snort 和 Suricata 等引擎的入侵检测规则的 Go 库。
...
- [Suricata Language Server](https://github.com/StamusNetworks/suricata-language-server) - Suricata 签名的语言服务器协议的实现。配置后，它会向您的首选编辑器添加语法检查、提示和自动完成功能。
...
- [Suricata-Check](https://suricata-check.teuwen.net/readme.html) - 一个命令行实用程序，用于提供有关 Suricata 规则的反馈。它可以检测覆盖语法有效性、可解释性、规则特异性、规则覆盖率和效率等问题。

## 文档和指南

- [SEPTun](https://github.com/pevma/SEPTun) - Suricata 极限性能调优指南。
- [SEPTun-Mark-II](https://github.com/pevma/SEPTun-Mark-II) - Suricata 极限性能调优指南 - Mark II。
- [suricata-4-analysts](https://github.com/StamusNetworks/suricata-4-analysts) - Suricata 安全分析师指南。
- [Suricata Community Style Guide](https://github.com/sidallocation/suricata-style-guide) - 从规则编写者社区收集样式指南的协作文档。


## 分析工具

- [Suricata Analytics](https://github.com/StamusNetworks/suricata-analytics) - 与 Suricata 数据交互时有用的各种资源。
- [Malcolm](https://github.com/cisagov/Malcolm) - 一个功能强大、易于部署的网络流量分析工具套件，用于完整的数据包捕获工件（PCAP 文件）、Zeek 日志和 Suricata 警报。
...
- [QuadrantSec Suricata Rules](https://github.com/quadrantsec/suricata-rules) - QuadrantSec 发布的 Suricata 规则集。
- [Cluster25/detection](https://github.com/Cluster25/detection) - Cluster25的检测规则。
- Networkforensic.dk (NF) 规则集：
  - [NF IDS 规则](https://networkforensic.dk/SNORT/NF-local.zip)
  - [NF SCADA IDS 规则](https://networkforensic.dk/SNORT/NF-SCADA.zip)
  - [NF 扫描仪 IDS 规则](https://networkforensic.dk/SNORT/NF-Scanners.zip)
...
- [3CORESec NIDS - Sinkholes](https://dtection.io/ruleset/3cs_sinkholes) - Suricata 规则集重点关注公共恶意软件陷阱的精选列表（免费）。
- [PAW Patrules](https://pawpatrules.fr) - Suricata 引擎的另一个免费 (CC BY-NC-SA) 规则集合。
- [opnsense-suricata-nmaps](https://github.com/aleksibovellan/opnsense-suricata-nmaps) - 针对 NMAP 扫描的 OPNSense Suricata IDS/IPS 检测规则。
- [Antiphishing](https://github.com/julioliraup/Antiphishing) - 用于检测网络钓鱼攻击的 Suricata 规则和数据集。
...
- [IOCmite](https://github.com/sebdraven/IOCmite) - 用于使用 MISP 实例指标创建 Suricata 数据集的工具，并在数据集指标生成警报时在 MISP 中添加目击事件。
- [luaevilbit](https://github.com/regit/luaevilbit) - Suricata 的 luajit 中的邪恶位实现。
- [Lawmaker](https://www.3coresec.com/solutions/lawmaker) - Suricata IDS 规则和车队管理系统。
...
- [Aristotle](https://github.com/secureworks/aristotle) - 简单的 Python 程序，允许根据每个规则内元数据关键字中存在的解释键值对过滤和修改 Suricata 和 Snort 规则集。


## 插件和扩展

- [suricata-zabbix](https://github.com/catenacyber/suricata-zabbix) - Suricata 的 Zabbix 应用层插件。


## 使用 Suricata 的系统

- [SELKS](https://github.com/StamusNetworks/SELKS) - 一个基于Suricata的入侵检测系统/入侵防御系统/网络安全监控分发。
- [Amsterdam](https://github.com/StamusNetworks/Amsterdam) - 基于 Docker 的 Suricata、Elasticsearch、Logstash、Kibana、Scirius 又名 SELKS。
- [pfSense](https://www.pfsense.org) - 免费的网络防火墙发行版，基于具有自定义内核的 FreeBSD 操作系统，并包含用于附加功能的第三方免费软件包。
- [OPNsense](https://opnsense.org) - 一个开源、易于使用且易于构建的基于 FreeBSD 的防火墙和路由平台。
- [Artica](https://github.com/dtouzeau/artica-suricata) - [Artica](https://artica.systems) 网关设备的 Suricata IDS 集成。
- [Shovel](https://github.com/FCSC-FR/shovel) - 用于探索 Suricata EVE 输出的 Web 界面，主要关注 CTF 比赛中的网络分析。


## 培训

- [Experimental Suricata Training Environment](https://github.com/jasonish/experimental-suricata-training) - 基于 Docker(-Compose) 的 Suricata 训练环境。
- [CDMCS](https://github.com/ccdcoe/CDMCS/tree/master) - 网络防御监控课程：基于规则的威胁检测。


## 模拟与测试

- [Leonidas](https://github.com/WithSecureLabs/leonidas) - 云中的自动攻击模拟，包含检测用例。
...
- [Suriwire](https://github.com/regit/suriwire) - 用于显示 Suricata 分析信息的 Wireshark 插件。
...
- [SuriGuard](https://github.com/SEc-123/SuriGuard1) - Suricata IDS/IPS 基于 Web 的管理系统，具有高级分析和可视化功能。
