...
# 很棒的TD引擎 [![Awesome](https://cdn.rawgit.com/sindresorhus/awesome/d7305f38d29fed78fa85652e3a63e154dd8e8829/media/badge.svg)](https://github.com/sindresorhus/awesome)

> 🎉 与 TDengine 相关的精彩内容的精选列表。

适用于时间序列数据的开源、高性能简化解决方案

TDengine 是一个开源的云原生时间序列数据库 (TSDB)，针对物联网 (IoT)、联网汽车和工业物联网进行了优化。它每天能够高效、实时地摄取、处理和监控由数十亿个传感器和数据收集器生成的 PB 级数据。凭借其内置的缓存、流处理和数据订阅功能，TDengine 为时序数据处理提供了简化的解决方案。

GitHub 存储库：https://github.com/taosdata/TDengine
...
- [SQLAuto](https://github.com/TommyLemon/SQLAuto) - 一个智能的数据库SQL测试自动化工具，支持任何CRUD、任何模板变量、生成参数组合、生成大量数据行。

## 物联网

- [Zeus IoT](https://github.com/zmops/zeus-iot) - Zeus IoT是全球首个基于Zabbix的开源物联网分布式采集平台，能够采集、分析和存储数百万物联网设备的数据。
...
- [node-red-contrib-tdengine](https://github.com/kp45-tech/node-red-contrib-tdengine) - Node-RED 的 TDengine 连接器插件。
...
## ORM
- [sqli](https://github.com/x-ream/sqli) - ORM SQL 接口、Criteria、CriteriaBuilder、ResultMapBuilder。
...
- [tdengine-orm](https://github.com/Yurunsoft/tdengine-orm) - TDengine ORM 基于 tdengine-restful-connector 和 php-tdengine。<!--lint 忽略 Awesome-list-item-->
- [crown](https://github.com/machine-w/crown) - Crown 是 TDengine 的一个简单且小型的 ORM。
- [APIJSON](https://github.com/Tencent/APIJSON) - 一个 JSON 传输协议和一个 ORM 库，用于提供 API 和文档，而无需编写任何代码。
...
- [DBUtility.TDengine](https://github.com/cockroach888/GSA.MOLLE.ToolKits/tree/main/src/ToolKits.DBUtility) - TDengine 的 C# 连接器。 （.NET 6+）。
...
- [taos-odbc](https://github.com/freemine/taos_odbc) - TDengine 3.0 的 ODBC 驱动程序，还处于早期阶段。
...
- [tsarchive](https://github.com/cenc-cea/tsarchive) - 消费来自Kafka主题的数据流，将数据包归档到TDengine时序数据库中。
- [flink-connector-tdengine](https://github.com/echisan/flink-connector-tdengine)
- [TD-Spark](https://github.com/liuyq-617/TD-Spark) - 一个java项目，在Spark上从TDengine读取并写入TDengine。
...
- [YoMo x TDengine](https://github.com/fanweixiao/push-stream-logging) - 将高频日志流式传输到 TDengine。<!--lint 忽略 Awesome-list-item-->
- [taostd](https://github.com/nzhzds/taostd) - taostd 是 TDengine 的简单 sql 执行器。

## 算法

- [PyODDS](https://github.com/datamllab/pyodds) - 端到端异常值检测系统。
- [PYTSDA-TDengine](https://github.com/Shawshank-Smile/PYTSDA-TDengine) - 使用 TDengine 的端到端时间序列数据分析系统。

## 可观察性

- [用于 n9e 传输的 TDengine 接口](https://github.com/xiangxud/transfer)
- [Business metrics monitoring based on TDengine](https://github.com/gunnerliu/horus) - 使用 TDengine 和 Spring Boot 进行高度定制的业务指标监控

## 集装箱

- [TDengine cluster for kubernetes](https://github.com/wwbgo/tdengine-k8s) - 适用于 kubernetes 的 TDengine 集群。
- [TDengine Containers](https://github.com/arktos-venture/docker-tdengine) - TDengine 客户端、节点、适配器和仲裁器尽可能无根且最小的容器。
- [TDengine-K8S](https://github.com/elihe2011/TDengine-K8S)

## 分布

- [tdengine-client-macos-unofficial](https://github.com/cybartists/tdengine-client-macos-unofficial) - 适用于 macOS 的非官方编译的 TDengine 客户端。
- [Unofficial TDengine Windows](https://github.com/GitHubForzhenjiazhao/Soft) - 非官方 TDengine Windows 客户端和服务器包。

## 数据集

- [seedlink2taos_py](https://github.com/schenton/seedlink2taos_py) - 一个Python程序，从IRIS获取地震波形数据并写入TDengine数据库。
- [airQuality](https://github.com/233lawliet/airQuality) - 3000+区域空气质量指标：CO2、CO1、PM2.5、PM10等。

## 其他

- [auto_taos_cfg](https://github.com/glzhao89/auto_taos_cfg) - 自动生成TDengine日志/数据/cfg。
- [Sentinel Console](https://github.com/wenhao/sentinel-dashboard-tdengine) - 带 TDengine 的 Sentinel-Dashboard。
