# 很棒的领域驱动设计 [![Awesome](https://cdn.rawgit.com/sindresorhus/awesome/d7305f38d29fed78fa85652e3a63e154dd8e8829/media/badge.svg)](https://github.com/sindresorhus/awesome)

> 领域驱动设计 (DDD)、命令查询职责分离 (CQRS)、事件溯源和事件风暴资源的精选列表。

**查看我的[博客](https://buildplease.com) 和[每周 DDD 时事通讯](https://buildplease.com/pages/dddweekly/) 或在 [Twitter](https://twitter.com/heynickc) 上打个招呼！**

**[领域驱动设计 (DDD)](https://en.m.wikipedia.org/wiki/Domain-driven_design)** 是一种通过将实现连接到不断发展的模型来满足复杂需求的软件开发方法。  领域驱动设计的前提如下：
...
**[命令查询职责分离 (CQRS)](http://codebetter.com/gregyoung/2010/02/16/cqrs-task-based-uis-event-source-agh/)** 只是创建两个对象，而以前只有一个对象。分离的发生取决于方法是命令还是查询（Meyer 在命令和查询分离中使用的定义相同，命令是改变状态的任何方法，查询是返回值的任何方法）。

**[事件溯源](http://www.martinfowler.com/eaaDev/EventSourcing.html)** 事件溯源的基本思想是确保应用程序状态的每个更改都被捕获在事件对象中，并且这些事件对象本身按照它们应用的顺序存储，与应用程序状态本身具有相同的生命周期。

**[事件风暴](https://ziobrando.blogspot.com/2013/11/introducing-event-storming.html)**是一种用于快速探索复杂业务领域的研讨会形式。

## 内容

- [很棒的领域驱动设计](https://github.com/heynickc/awesome-ddd/)
	- [Contents](#contents)
	- [Books](#books)
	- [免费电子书](#free-ebooks)
...
- [CQRS](https://leanpub.com/cqrs) - Mark Nijhof 撰写的笔记，记录了他向 Greg Young 学习 DDD 和 CQRS 的经历。  本书附带有一个广泛的示例项目。
- [Domain-Driven Design: Tackling the Complexity at the Heart of Software](https://amzn.com/0321125215) - 创造领域驱动设计一词的经典书籍。  又称“蓝皮书”。
- [Domain-Driven Design Distilled](https://www.amazon.com/Domain-Driven-Design-Distilled-Vaughn-Vernon/dp/0134434420) - 在阅读[实现领域驱动设计](https://vaughnvernon.co/?page_id=168#iddd)或[领域驱动设计：解决软件核心的复杂性](https://amzn.com/0321125215)之前，这是一本非常好的入门书。
- [Domain-Driven Design in PHP](https://leanpub.com/ddd-in-php) - 用 PHP 编写的真实示例展示了 DDD 架构风格、战术设计和限界上下文集成。
- [Domain-Driven Rails](https://blog.arkency.com/domain-driven-rails/) - 领域驱动的 Rails 描述了 11 种技术（从限界上下文到事件溯源），您可以在新旧 Rails 应用程序中单独或一起使用这些技术来实现更好的架构。
- [Domain Modeling Made Functional](https://pragprog.com/book/swdddf/domain-modeling-made-functional) - 使用领域驱动设计和 F# 解决软件复杂性。
- [Domain Specific Languages](http://martinfowler.com/books/dsl.html) - 与 DDD 相关，描述了通用语言和与领域专家合作的重要性，以及许多技术细节。
...
- [Hands-On Domain-Driven Design with .NET Core](https://www.amazon.com/Hands-Domain-Driven-Design-NET-dp-1788834097/dp/1788834097) - Alexey Zimarev 通过将 DDD 原则付诸实践来解决软件核心的复杂性。
- [Implementing Domain-Driven Design](https://www.amazon.com/Implementing-Domain-Driven-Design-Vaughn-Vernon/dp/0321834577) - 这也是一本规范的书，介绍了一种自上而下的方法来理解领域驱动设计。  又称“红皮书”。
...
- [Learning Domain-Driven Design: Aligning Software Architecture and Business Strategy](https://www.amazon.com/Learning-Domain-Driven-Design-Aligning-Architecture/dp/1098100131/) - 了解领域驱动设计的基本模式和实践，以及如何将它们应用到日常工作中，无论是新建项目还是棕地项目。
...
- [Patterns, Principles, and Practices of Domain-Driven Design (1st Edition)](https://www.amazon.com/Patterns-Principles-Practices-Domain-Driven-Design/dp/1118714709) - 遵循领域驱动设计的实践、原则和模式以及 C# 代码示例来管理复杂软件构建的方法。
...
- [Versioning in an Event Sourced System](https://leanpub.com/esversioning) - 您在对事件溯源系统进行版本控制时遇到过麻烦吗？刚刚第一次进入新的事件源系统？本书是关于如何长期处理版本控制的权威指南。
...
- [Writing Great Specifications](https://www.manning.com/books/writing-great-specifications) - 《编写出色的规范》是一个示例丰富的教程，教您如何编写良好的 Gherkin 规范文档，以利用示例规范的优点。
- 

## 免费电子书

- [The Anatomy of Domain-Driven Design - The Infographic](https://leanpub.com/theanatomyofdomain-drivendesign) - 关于领域驱动设计剖析的信息图。
- [Architecting Modern Web Applications with ASP.NET Core and Microsoft Azure](https://dotnet.microsoft.com/download/thank-you/aspnet-ebook) - 提供有关使用 DDD、ASP.NET Core 和 Azure 构建整体式 Web 应用程序的端到端指南。 [eShopOnWeb 示例](https://github.com/dotnet-architecture/eShopOnWeb)
...
- [Domain-Driven Design: The First 15 Years](https://dddeurope.com/15years) - 为了庆祝周年纪念，我们邀请软件设计界的著名作者贡献新旧论文。 Martin Fowler、James Coplien、Rebecca Wirfs-Brock、Mel Conway 等人做出了贡献。
- [DDD Reference](http://domainlanguage.com/wp-content/uploads/2016/05/DDD_Reference_2015-03.pdf) - DDD 模式和定义的摘要。
...
- [.NET Microservices: Architecture for Containerized .NET Applications](https://dotnet.microsoft.com/download/thank-you/microservices-architecture-ebook) - 介绍如何开发基于微服务的应用程序并使用容器管理它们。 [eShopOnContainers 示例](https://github.com/dotnet-architecture/eShopOnContainers)
...
- [Visual Collaboration Tools](https://leanpub.com/visualcollaborationtools/) - 可视化协作工具是一本针对团队构建软件的书。它描述了帮助我们完成日常工作的工具，并展示了不同从业者的现场故事。

## 培训课程
- [Advanced Distributed Systems Design](https://learn-particular.thinkific.com/courses/adsd-online) - Udi Dahan 的在线培训课程。  Udi 的现场训练时间表可以在[此处](http://udidahan.com/training/) 找到。
...
- [Distilling Domain-Driven Design](https://www.informit.com/store/domain-driven-design-livelessons-video-training-9780134597324) - 沃恩·弗农的在线培训课程。
...
- [EventStorming 基础知识课程](https://elearning.dsolemorera.com/courses/eventstorming-fundamentals)（[也为西班牙语](https://elearning.dsolemorera.com/courses/fundamentos-eventstorming)）- 学习 EventStorming 的基础知识。有关 EventStorming 的所有内容以及示例。
...
- [Pluralsight](https://pluralsight.com)
	 ...
	 - [Modern Software Architecture: Domain Models, CQRS, and Event Sourcing](https://www.pluralsight.com/courses/modern-software-architecture-domain-models-cqrs-event-sourcing) - 本课程涵盖有助于发现业务领域的顶层架构的 DDD 分析模式。架构师和开发人员将找到实现模式的详细信息，例如域模型、CQRS 和事件溯源。
...
- [Alberto Brandolini: Event Storming](https://www.youtube.com/watch?v=veTVAN0oEkQ&list=PLve553MhJLs4YkEnHmOjWJv0B-6WY0-JI) - YouTube 上阿尔贝托·布兰多里尼 (Alberto Brandolini) 关于事件风暴的演讲合集。
...
- [Virtual Domain-driven design](https://www.youtube.com/channel/UCob_jOzzpxBp-di-x0vLlwA) - 虚拟域驱动设计中录制的聚会会议的 YouTube 集合
- [Visuality DDD webinars](https://youtube.com/playlist?list=PLvMNoWK93wtnu9JcSEYnFRgzqbxtLtZZ4) - YouTube 网络研讨会合集，重点关注 Ruby on Rails 中的 DDD
- [The Art of Discovering Bounded Contexts by Nick Tune](https://www.youtube.com/watch?v=ez9GWESKG4I) - 2017 年 DevoxxUK 会议关于如何定义有界上下文

## 社区资源

- [DDD/CQRS Google Group](https://groups.google.com/forum/?utm_source=digest&utm_medium=email#!forum/dddcqrs) - 活跃的邮件列表和优秀的资源，用于提出问题和了解有关 DDD/CQRS 的详细信息。
...
- [DDD in Ruby on Rails](https://www.visuality.pl/posts/introduction-to-ddd-in-ruby-on-rails) - 有关 Ruby on Rails 中的领域驱动设计的文章集
- [DDD in Ruby subreddit](https://www.reddit.com/r/ddd_ruby/) - 针对领域驱动设计感兴趣的 Ruby 开发人员的 Reddit 子版块。
...
- [Domain StoryTelling Discord](https://discord.gg/KsYaHNNn33) - 一个关于领域故事讲述（#domain-storytelling 频道）的 Discord 团队，作者：[@hofstef](https://twitter.com/hofstef)。主页位于 [domainstorytelling.org](http://domainstorytelling.org/)。
- [Software Engineering Stack Exchange](http://softwareengineering.stackexchange.com/questions/tagged/domain-driven-design) - 软件工程 Stack Exchange 问题标记为*领域驱动设计*。
...
- [Advanced Topics in Event Sourcing / CQRS / DDD](https://github.com/sebastianharko/adv-es-cqrs-ddd) - 事件溯源 / CQRS / DDD 高级主题列表由 @[sebastianharko](https://github.com/sebastianharko) 编译和维护。
- [Quora](https://www.quora.com/topic/Domain-Driven-Design-DDD) - 标记为*领域驱动设计*的问题。
- [wolkenkit Slack](http://slackin.wolkenkit.io/) - 一个致力于 DDD、事件溯源、CQRS 和 wolkenkit 的 Slack 团队。
...
- [Domain-Driven Design in dynamic languages GitHub](https://github.com/valignatev/ddd-dynamic) - GitHub 上关于动态语言领域驱动设计的简介。
...
- [DDD North America](https://dddna.net/) - 北美各地即将举行的现场培训、活动和用户组。

## 博客

- [Aardling Blog](https://aardling.eu/en/insights) - Mathias Verraes 等人的 DDD 和软件设计文章。
- [Nick Chamberlain](https://buildplease.com) - 为 .NET 开发人员提供有用的开发和设计建议。
- [Ardalis.com](https://ardalis.com/blog) - 史蒂夫·史密斯。 Pluralsight 作者，[DDD 基础知识](https://www.pluralsight.com/courses/domain-driven-design-fundamentals) 课程和 Microsoft [eShopOnWeb 示例应用程序](https://github.com/dotnet-architecture/eShopOnWeb) 的作者。
- [DDD Weekly](http://dddweekly.com) - 每周精选与 DDD/CQRS/ES 相关的链接。
- [Daniel Whittaker](http://danielwhittaker.me) - 想要了解 CQRS 和事件溯源吗？该博客包含分步文章，可帮助您抢占先机。
- [Cyrille Martraire](http://cyrille.martraire.com) - 由于对金融如此痴迷，同时仍然热爱编程，我自然是 Eric Evans 的领域驱动设计以及 TDD、BDD 和敏捷/XP 实践的忠实粉丝。
...
- [Christian Posta](http://blog.christianposta.com) - 首席中间件架构师 @ Red Hat、开源爱好者、提交者 @ Apache、Cloud、Integration、Kubernetes、Docker、OpenShift、Fabric8。
- [Vladimir Khorikov](http://enterprisecraftsmanship.com) - 多元视野作者。  有关软件开发原则和最佳实践的博客。
...
- [Jérémie Chassaing](http://thinkbeforecoding.com/) - 有关 DDD/CQRS 的各种文章。  在 F# 中实现了 Greg Young 的 SimpleCQRS 示例。
...
- [Eventsourcing Publications](https://blog.eventsourcing.com) - 实用事件溯源。
...
- [Roger Johansson](https://rogeralsing.com/) - 主要是 C#、DDD 和 Akka.NET。
- [Konrad Garus](http://squirrel.pl/blog/) - 使用 Java、Clojure 和 JavaScript 进行咆哮和编程。  带有 *cqrs* 标签的文章。
- [Oasis Digital](http://blog.oasisdigital.com/category/cqrs/) - Oasis Digital 的内容在 *cqrs* 下标记。
...
- [Lev Gorodinski](http://gorodinski.com/) - 2013 年的几篇关于 DDD 的文章仍然具有相关性。
- [Dino Esposito](https://software2cents.wordpress.com/) - 软件架构师、培训师、书籍作者。  [Microsoft .NET - 构建企业应用程序（第二版）](https://www.amazon.com/Microsoft-NET-Architecting-Applications-Enterprise/dp/0735685355/) 的作者。
...
- [Martin Havlišta](https://xhafan.com/blog/) - DDD、CQRS、TDD 博客文章以及 C# .NET 中的代码示例
- [Khalil Stemmler](https://khalilstemmler.com/articles/categories/domain-driven-design) - DDD系列介绍并谈如何用TypeScript实现DDD
- [Kenny Baas-Schwegler](https://baasie.com/) - DDD、BDD、社会技术、EventStorming 和持续交付博客。
- [João Rosa](https://joaorosa.io) - 关于领域驱动设计、视觉协作、领导力和组织设计的个人博客。以及介于两者之间的其他事情。 [视觉协作工具](https://leanpub.com/visualcollaborationtools/) 策展人和[软件工艺播客](https://www.softwarecraftspodcast.com/) 主持人
- [GlobalAppTesting engineering](https://gat.engineering) - GlobalAppTesting 的工程博客，提供有关实用 DDD 和 CQRS 的材料。

## 示例项目

### GO
- [BDD in GO](https://github.com/JankariTech/bsDateServer) - 示例应用程序演示了如何使用 Cucumber + GO 进行 BDD 测试方法。博客文章可以在[此处](https://dev.to/jankaritech/demonstrating-bdd-behavior-driven-development-in-go-1eci)找到。
- [Citerus DDD Sample App GO Port](https://github.com/marcusolsson/goddd) - 这是将 [DDD 示例应用程序](https://github.com/citerus/dddsample-core) 移植到惯用的 Go 的尝试。它可以在 dockerized 模式下运行以预览应用程序。
- [DDD by Refactoring](https://github.com/ThreeDotsLabs/wild-workouts-go-ddd-example) - 完整的无服务器应用程序，展示如何通过 Go 项目的实际重构来应用 DDD、Clean Architecture 和 CQRS。有关它的完整博客系列可以在 <https://thirddots.tech/> 找到。
- [DDD Food App](https://github.com/victorsteven/food-app-server) - 示例 DDD 应用程序实现 4 层（域、基础设施、应用程序和接口）并考虑两个域模式。 [此处]（https://dev.to/stevensunflash/using-domain-driven-design-ddd-in-golang-3ee5）为此编写了一篇博客文章。
- [DDD Sample in GO](https://github.com/takashabe/go-ddd-sample) - 这只是另一个实现 DDD 四层的示例应用程序。
- [Evolutive CRUD API](https://github.com/friendsofgo/gopherapi) - 使用 SOLID、六边形架构实现具有完整 CRUD 的 API 实现。 <https://blog.friendsofgo.tech/> 上有一系列为此撰写的博客文章。
- [Simple Hexagonal Architecture PoC API](https://github.com/tomiok/patients-API) - 使用六边形架构模式的患者 API 的 PoC。
- [Azure DDD boilerplate](https://github.com/joshpme/azure-go-ddd-boilerplate) - Azure 中 DDD 的样板项目，使用自定义处理程序和 Cosmos DB 进行事件溯源
...
- [Companion Code for Microsoft .NET Architecting Applications for the Enterprise](https://github.com/mastreeno/Merp) - 基于事件的 Micro ERP。
...
- [DDDInventoryItemFSharp](https://github.com/eulerfx/DDDInventoryItemFSharp) - 领域驱动设计的惯用 F# 实现
- [DDDSkeletonNet](https://github.com/andras-nemes/DDDSkeletonNet) (C#) - 一个 .NET 框架项目，用于介绍领域驱动设计和松散耦合层的概念。
- [DotNet CQRS Intro](https://github.com/asc-lab/dotnet-cqrs-intro) - 使用事件源实现 CQRS 的示例 - 进化方法（无 CQRS、具有相同模型的单独模型和命令、具有单独模型的单独模型和命令、单独的存储引擎、事件源）。
- [EISK](https://github.com/eisk) - .NET CLI 和 VS 模板具有简单的用例，可通过架构最佳实践（DDD、洋葱架构等）在 .net core 上构建可扩展的应用程序。
- [EmailMaker](https://github.com/xhafan/emailmaker) - 电子邮件营销 ASP.NET Core MVC 和 ASP.NET MVC 演示应用程序演示 [CoreDdd](https://github.com/xhafan/coredd) 用法
- [Equinox Project](https://github.com/EduardoPires/EquinoxProject) - 完整的 ASP.NET Core 3.1 应用程序，具有简洁架构、DDD、CQRS 和事件源概念
- [eShopOnWeb](https://github.com/dotnet-architecture/eShopOnWeb) - Microsoft 的完整 ASP.NET Core 3.1 参考应用程序显示了整体部署架构
...
- [EventFlow.Example](https://github.com/OKTAYKIR/EventFlow.Example) - DDD、CQRS 和事件源示例，包含以下技术堆栈：[EventFlow](https://github.com/eventflow/EventFlow)、[EventStore](https://eventstore.com)、[RabbitMQ](https://www.rabbitmq.com)、[MongoDB](https://www.mongodb.com)、[PostgreSQL](https://www.postgresql.org)、[Docker](https://www.docker.com)
- [Fohjin](https://github.com/MarkNijhof/Fohjin) - Mark Nijhof 的 [CQRS](https://leanpub.com/cqrs) 书中附带的示例项目。
- [FsUno](https://github.com/thinkbeforecoding/FsUno) - F# 中的事件溯源实现示例。
- [IDDD Samples in .NET](https://github.com/VaughnVernon/IDDD_Samples_NET) - 这些是 Vaughn Vernon 所著的《实现域驱动设计》一书中的 C#.NET 限界上下文示例。
- [LexiQuest-Modular-DDD](https://github.com/ryletko/LexiQuest-Modular-DDD) - 采用清洁架构和 DDD 原则构建的模块化应用程序，可快速拆分为微服务。
...
- [Modular.StarterTemplate](https://github.com/ryletko/Modular.StarterTemplate) - 采用清洁架构 DDD 风格的模块化应用程序的入门模板，在单个事务中模块之间具有同步交互。非常适合 ERP 应用。
- [NLayerAppV3](https://github.com/cesarcastrocuba/nlayerappv3) (.Net Core Preview 2) - N 层架构示例项目。
- [照片库 CQRS-DDD 示例](https://github.com/mr0zek/Photostock)
...
- [Scritchy](https://github.com/ToJans/Scritchy) - 没有管道的 CQRS，[视频](http://www.youtube.com/watch?v=5DKTFZD3hu8)。
- [Simple CQRS in F#](https://github.com/thinkbeforecoding/m-r) - Greg Young 的 F# 中的 SimpleCQRS。
...
- [TodoMVC-DDD-CQRS-EventSourcing](https://github.com/volak/TodoMVC-DDD-CQRS-EventSourcing) - 通过tastejs/todomvc在C#/Typescript中使用事件源、cqrs和领域驱动设计实现基本的Todo应用程序

### 哈斯克尔
- [Eventuria gsd](https://github.com/Eventuria/gsd) - Haskell 待办事项列表包含 DDD、CQRS 和事件源、FRP 的反应式应用程序。

### 伊德里斯
- [OrderTaking](http://github.com/andorp/order-taking) - 《领域建模变得实用》一书的依赖类型实现。一个示例项目，如何使用依赖类型形式化有界上下文和工作流程图。 NodeJS 可部署演示。
...
- [Booster 框架示例](https://github.com/boostercloud/booster/tree/master/docs/examples) 使用 Booster Framework 构建的示例应用程序。
- [Over-engineered ToDo app](https://github.com/bitloops/ddd-hexagonal-cqrs-es-eda) - 使用领域驱动设计 (DDD)、六边形架构、CQRS、事件溯源 (ES)、事件驱动架构 (EDA)、使用 TypeScript 进行行为驱动开发 (BDD) 以及使用 [Bitloops 语言 (BL)](https://github.com/bitloops/bitloops-language) 生成的 NestJS 的完整工作示例。

### JVM 语言
- [Akka CQRS ES Demo](https://github.com/mdonkers/akka-cqrs-es-demo) - 在 Scala-Akka 中实现 CQRS 和事件溯源模式的演示项目。
- [DDD By Examples - Library](https://github.com/ddd-by-examples/library) - 由实际业务需求驱动的库的示例项目。在 DDD、BDD、EventStorming、示例映射、CQRS 等的帮助下实现的模块化整体。
- [DDD Leaven](https://github.com/BottegaIT/ddd-leaven-v2) - DDD-CQRS 示例 v2.0 项目可帮助您使用 Spring、JPA 和测试开始高级域建模。
- [DDD Workshop - Project Manager](https://github.com/mkopylec/project-manager) - 同时举办“Do It Yourself”DDD 研讨会和示例 DDD 应用程序。基于项目管理域。
- [Event Sourcing Example](https://github.com/Pragmatists/eventsourcing-java-example) - Java 代码的事件溯源和 CQRS 实现的简化（在内存中）示例（针对银行领域用例建模）。
- [Event Sourcing and CQRS Examples](https://github.com/andreschaffer/event-sourcing-cqrs-examples) - 事件源和 CQRS 在 Java 中的实用应用，为常见相关问题提供了很好的参考，例如事件排序和幂等性。
...
- [Java CQRS Intro](https://github.com/asc-lab/java-cqrs-intro) - 使用事件源实现 CQRS 的示例 - 进化方法（无 CQRS、具有相同模型的单独模型和命令、具有单独模型的单独模型和命令、单独的存储引擎、事件源）。
...
- [Eclipse CargoTracker](https://eclipse-ee4j.github.io/cargotracker/) - 该项目演示了如何使用领域驱动设计 (DDD) 等广泛采用的架构最佳实践，通过 Jakarta EE 平台开发应用程序。 [代码](https://github.com/eclipse-ee4j/cargotracker/) 旨在镜像开发人员在实际工作中将处理的重要应用程序。它试图直接演示如何使用 Jakarta EE 来有效地满足企业的实际问题，例如生产力、敏捷性、可测试性、灵活性、可维护性、可扩展性和安全性。该项目直接基于 DDD 先驱 Eric Evans 的公司 Domain Language 和瑞典软件咨询公司 Citerus 开发的著名原创 [Java DDD 示例应用程序](https://github.com/citerus/dddsample-core)。
...
- [DDD Modulith](https://github.com/janikredpandadev/ddd-modulith) - 以 Symfony 7 作为模块的 DDD 洋葱架构实现。
...
- [GoConvey](https://github.com/smartystreets/goconvey) - 去浏览器中测试一下。与“go test”集成。用 Go 编写行为测试。
- [Godog](https://github.com/cucumber/godog) - godog 包是 Golang 的官方 Cucumber BDD 框架，它将规范和测试文档合并为一个整体，使用 Gherkin 格式的场景（Given、When、Then 格式）。
...
- [ASP.NET Boilerplate](http://aspnetboilerplate.com/) - 基于 ASP.NET MVC、Web API 和 ASP.NET Core 的应用程序框架，用于创建实现最佳实践的多层、域驱动设计的 Web 应用程序。
...
- [Azure 上的 CQRS](https://github.com/MerrionComputing/CQRSAzure) Windows Azure 上的 CQRS。
...
- [EventFlow](https://github.com/eventflow/EventFlow) - Async/await 第一个用于 .NET 的 CQRS+ES 和 DDD 框架 http://geteventflow.net/。
- [Core.EventStore](https://github.com/younos1986/Core.EventStore) - 一个促进 CommandService 和 QueryService 之间通信的库。想法是当commandService中发生任何事件时，它应该被持久化在MongoDb中的QueryService中。
...
- [MediatR](https://github.com/jbogard/MediatR) - 支持请求/响应、命令、查询、通知和事件、同步和异步，并通过 C# 通用方差进行智能调度。
- [MessageRouter](https://github.com/QuickenLoans/MessageRouter) - 本视频中介绍了：纽约 F# .NET 用户组的 Paulmichael Blasucci 的[CQRS 的跳动之心，或 CLR 上基于 Actor 的消息路由](https://vimeo.com/171178586)。
...
- [Deveel Repository](https://github.com/deveel/deveel.repository) - .NET 存储库模式的简单实现，支持 MongoDB 和实体框架，使用更多实用程序（缓存、分页、验证等）扩展模型。

### 数据库
- [Event Store](https://geteventstore.com) - 开源功能数据库，具有 JavaScript 中的复杂事件处理功能。
- [Eventsourcing](https://eventsourcing.com) - 业务事件捕获和查询框架。
- [Message DB](https://github.com/message-db/message-db) - Postgres 的微服务本机事件存储和消息存储。在 PostgreSQL 中实现的功能齐全的事件存储和消息存储，用于 Pub/Sub、事件源、消息传递和事件微服务应用程序。
- [Serialized](https://serialized.io) - 事件溯源和 CQRS 的完整平台。

### 灵丹妙药
- [Commanded](https://github.com/slashdotdash/commanded) - Elixir (Erlang VM) 中的 CQRS/ES 应用程序的命令处理中间件、聚合和流程管理器的纯功能数据结构、点对点消息路由等 - 全部都在 Actor 并发模型中。
...
- [wolkenkit](https://www.wolkenkit.io/) - 适用于 JavaScript 和 Node.js 的 CQRS、DDD 和事件溯源框架。
...
- [Axon Framework](http://www.axonframework.org/) - axon 框架致力于让想要基于 CQRS 原则创建 Java 应用程序的开发人员的生活变得更轻松。
...
- [Ahoo-Wang/Wow](https://github.com/Ahoo-Wang/Wow) - 基于 DDD 和 EventSourcing 的现代反应式 CQRS 架构微服务开发框架。

### PHP
- [Broadway](https://github.com/broadway/broadway) - Broadway 是一个 (PHP) 项目，为创建 CQRS 和事件源应用程序提供基础架构和测试帮助程序。
- [CodefyPHP Framework](https://github.com/codefyphp/) - 一个 PHP 框架，用于使用领域驱动设计、CQRS 和事件源来编码和构建复杂的应用程序。
- [Ecotone](http://ecotone.tech) - 在 PHP 中启用消息驱动架构，并提供遵循 DDD 和 CQRS 原则的构建块。
- [PHP Glossary](https://github.com/javanile/php-glossary) - 应用术语分析以 DDD 方式提取领域术语与范围外术语。
...
- [探索 DDD - 美国](http://exploreddd.com/)
- [康丁斯基 - 德国](http://kandddinsky.com/)

## 用户组

- [Collective CFP](https://sessionize.com/ddd-meetups) - 将您的演讲一次性提交给所有 DDD 用户组。
- [欧洲用户群体地图](https://datawrapper.dwcdn.net/9FNZI/)
- [北美用户群体分布图](https://datawrapper.dwcdn.net/nbZkd/)
- [亚洲用户群体分布图](https://datawrapper.dwcdn.net/oin66/)
- [非洲用户群体地图](https://datawrapper.dwcdn.net/yaEOa/)
- [Virtual](https://virtualddd.com/)
- [Worldwide](https://www.meetup.com/worldwide-eventstorming-meetup/) - 有关 EventStorming 的具体信息
...
- [DDD台湾社区](https://www.facebook.com/groups/dddtaiwan/)
...
- [Domain Storytelling](http://www.domainstorytelling.org/) - 一种知识处理技术，可以帮助相关人员熟悉该领域并制定出表达他们共同理解的模型。可作为[打印模板](http://www.domainstorytelling.org/images/DST_Whiteboard-Kit.pdf)，以及开源在线工具[WPS Modeler](https://www.wps.de/modeler)（[来源](https://github.com/wps/domain-story-modeler)）。
