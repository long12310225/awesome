<!--lint disable double-link-->

# 很棒的账本 [![Awesome](https://awesome.re/badge.svg)](https://awesome.re)

> [Ledger](http://ledger-cli.org/) 命令行记账系统的有用资源

您可能还喜欢 [awesome-beancount](https://github.com/wzyboy/awesome-beancount)。

*贡献前请阅读[贡献指南](contributing.md)。*

## 内容

<!-- START doctoc generated TOC please keep comment here to allow auto update -->
<!-- DON'T EDIT THIS SECTION, INSTEAD RE-RUN doctoc TO UPDATE -->

- [Introduction](#introduction)
- [Background](#background)
- [Documentation](#documentation)
- [Editors](#editors)
- [Tools](#tools)
- [Social](#social)
- [Media](#media)
- [Miscellaneous](#miscellaneous)

<!-- END doctoc generated TOC please keep comment here to allow auto update -->

## 简介

- [Ledger](http://ledger-cli.org/) - 官方网站。
- [Wiki](https://github.com/ledger/ledger/wiki) - 官方维基。
- [维基百科](https://en.wikipedia.org/wiki/Ledger_(software)) - 关于 Ledger 的维基百科条目。

## 背景

- [Plain Text Accounting](http://plaintextaccounting.org/) - Ledger 背后的总体思路。
- [Wikipedia](https://en.wikipedia.org/wiki/Double-entry_bookkeeping_system) - 关于复式记账的维基百科条目。

## 文档

- [Cheat Sheet](http://ricostacruz.com/cheatsheets/ledger.html) - 最常用命令的摘要。
- [Book](https://github.com/rolfschr/GSWL-book) - Ledger 的入门书。
- [Manual](http://ledger-cli.org/3.0/doc/ledger3.html) - 官方手册。
- [man page](http://ledger-cli.org/3.0/doc/ledger.1.html) - 命令行选项的描述。

## 编辑
- [vim](https://github.com/ledger/vim-ledger) - Vim 插件。
- [Emacs](http://www.ledger-cli.org/3.0/doc/ledger-mode.html) - 账本模式。
- [Atom](https://atom.io/packages/language-ledger) - 原子包。
- [VS Code](https://github.com/mariosangiorgio/vscode-ledger) - Visual Studio Code 的 Ledger 语言支持。

## 工具

- [ledgerhelpers](https://github.com/Rudd-O/ledgerhelpers) - 帮助程序的集合。
- [node-ledger-web](https://github.com/slashdotdash/node-ledger-web) - 账本数据的 Web 前端。
- [ledger-autosync](https://github.com/egh/ledger-autosync) - 将分类帐文件与银行同步的程序。

## 社交

- [Twitter](https://twitter.com/LedgerTips) - 使用技巧。
- [Google Group](https://groups.google.com/forum/#!forum/ledger-cli) - 分类账谷歌集团。
- [Stack Exchange](https://money.stackexchange.com/search?q=ledger-cli) - 标签“个人理财和资金堆栈”。

## 媒体

- [FLOSS Weekly 150](https://twit.tv/shows/floss-weekly/episodes/150) - 采访开发商。
- [FLOSS Weekly 375](https://twit.tv/shows/floss-weekly/episodes/375) - 关于替代实施的采访。
- [Ledger + Emacs](https://www.youtube.com/watch?v=cjoCNRpLanY) - 介绍在 Emacs 中使用 Ledger。

## 杂项

- [Code](https://github.com/ledger/ledger) - GitHub 上的代码。
- [Ports](https://github.com/ledger/ledger/wiki/Ports) - 替代实现的列表。
